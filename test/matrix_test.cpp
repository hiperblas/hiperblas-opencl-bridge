...
TEST_F(MatrixFixture, large_matvec_mul3_WithComplex) {
    ...
    m.bridges[idx].vecreqhost(r);
    for (int i = 0; i < 2 * a->len; i += 2) {
        //printf("%d %4.18e %4.18e\n",i,truth->value.f[i],r->value.f[i]);
        //printf("%d %4.18e %4.18e\n",i,truth->value.f[i+1],r->value.f[i+1]);
...
// TEST_F(MatrixFixture, matrix_mul_larger_than_device_memory_real) {
...
//     for (int i = 0; i < b->ncol; i++) {
//         for (int j = 0; j < b->nrow; j++) {
//             a->value.f[i * a->ncol + j] = 1.;
//             b->value.f[i * b->ncol + j] = 1.;
//         }
//     }
